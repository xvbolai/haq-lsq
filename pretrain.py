...
# 设置要限制的显存比例
memory_fraction = 0.75  # 使用50%的显存

# 检查是否有可用的GPU
if torch.cuda.is_available():
    # 设置每个进程可使用的显存比例
    torch.cuda.set_per_process_memory_fraction(memory_fraction, device=torch.cuda.current_device())
else:
    print("没有可用的GPU设备。")
...
from lib.utils.quantize_utils import quantize_model, kmeans_update_model, QConv2d, LSQConv2d, LSQLinear, QLinear, calibrate, dorefa, set_fix_weight
...
parser.add_argument('--lr_min', type=float, default=0.0, help='Minimum learning rate. Default: 0')
...
if __name__ == '__main__':
    ...
    print("=> creating model '{}'".format(args.arch), ' pretrained is ', args.pretrained, ' bitlength is ', args.bits, ' observer step is ', args.observer_step)
    print('lr type', args.lr_type,' learning rate is ', args.lr, ' momentum is ', args.momentum, ' weight decay is ', args.weight_decay)
