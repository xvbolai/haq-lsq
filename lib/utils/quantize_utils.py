...
class LSQActQuantizer(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.quant or not self.bits > 0:
            return x
...
class LSQModule(QModule):
    def __init__(self, w_bit= 8, quant_wgt=True, wgt_qtype="qint", wgt_per_channel=False,
                 a_bit= -1, quant_act=True, act_qtype="quint",
                 quant=True, observer=True, learning=False, observer_step=2000, half_wave=True):
        super(LSQModule, self).__init__(w_bit=w_bit, a_bit=a_bit, half_wave=half_wave)
        
        self.quant = quant
        self.quant_wgt = quant_wgt
        self.quant_act = quant_act
        self.act_quantizer = LSQActQuantizer(a_bit, qtype=act_qtype, quant=quant, observer=observer, learning=learning)
        self.weight_quantizer = LSQWeightQuantizer(w_bit, qtype=wgt_qtype, per_channel=wgt_per_channel,
    ...
    def w_bit(self, w_bit):
        self._w_bit = w_bit
        self.weight_quantizer.set_bits(w_bit)
    ...
    def a_bit(self, a_bit):
        self._a_bit = a_bit
        self.act_quantizer.set_bits(a_bit)
...
class LSQConv2d(LSQModule):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=False,
                 w_bit= 8, quant_wgt=True, wgt_qtype="qint", wgt_per_channel=False,
                 a_bit= -1, quant_act=True, act_qtype="quint",
                 quant=True, observer=True, learning=False, observer_step=2000, half_wave=True):
        super(LSQConv2d, self).__init__(w_bit= w_bit, quant_wgt=quant_wgt, wgt_qtype=wgt_qtype, 
                                        wgt_per_channel=wgt_per_channel,
                 a_bit= a_bit, quant_act=quant_act, act_qtype=act_qtype,
                 quant=quant, observer=observer, learning=learning, 
                 observer_step=observer_step, half_wave=half_wave)
        if in_channels % groups != 0:
            raise ValueError('in_channels must be divisible by groups')
        if out_channels % groups != 0:
            raise ValueError('out_channels must be divisible by groups')
...
class LSQLinear(LSQModule):
    def __init__(self, in_features, out_features, bias=True, w_bit= 8, a_bit= 8, quant_wgt=True, 
                 wgt_qtype="qint", wgt_per_channel=False, quant_act=True, act_qtype="quint",
                 quant=True, observer=True, learning=False, observer_step=2000, half_wave=True):
        super(LSQLinear, self).__init__(w_bit= w_bit, quant_wgt=quant_wgt, wgt_qtype=wgt_qtype, 
                                        wgt_per_channel=wgt_per_channel,
                 a_bit= a_bit, quant_act=quant_act, act_qtype=act_qtype,
                 quant=quant, observer=observer, learning=learning, 
                 observer_step=observer_step, half_wave=half_wave)
    ...
    def forward(self, inputs):
        ...
        return F.linear(act, weight=wgt, bias=self.bias)
...
class BitPruningModule(QModule):
    def __init__(self, w_bit=8, a_bit=8, half_wave=True):
        super(BitPruningModule, self).__init__(w_bit=w_bit, a_bit=a_bit, half_wave=half_wave)
        
        # bit length
        self._a_bitlength = nn.Parameter(torch.tensor([a_bit], dtype=torch.float32), requires_grad=True)
        self._w_bitlength = nn.Parameter(torch.tensor([w_bit], dtype=torch.float32), requires_grad=True)
    ...
    def _q_v_n(self, inputs, n):
        minn = inputs.min()
        scale = (inputs.max() - minn) / (2 ** n - 1)
        int_V = Round.apply((inputs - minn) / scale) * scale
        return minn + int_V * scale
    ...
    def _quantize_activation(self, inputs):
        b = BitPruningModule.Floor.apply(self._a_bitlength)
        alpha =  self._a_bitlength - b
        output = (1 - alpha) * self._q_v_n(inputs, b) + alpha * self._q_v_n(inputs, b + 1)
        return output

    def _quantize_weight(self, weight):
        b = BitPruningModule.Floor.apply(self._w_bitlength)
        alpha =  self._w_bitlength - b
        output = (1 - alpha) * self._q_v_n(weight, b) + alpha * self._q_v_n(weight, b + 1)
        return output
...
class BitPruningConv2d(BitPruningModule):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=False,
                 w_bit=-1, a_bit=-1, half_wave=True):
        super(BitPruningConv2d, self).__init__(w_bit=w_bit, a_bit=a_bit, half_wave=half_wave)
        if in_channels % groups != 0:
            raise ValueError('in_channels must be divisible by groups')
        if out_channels % groups != 0:
            raise ValueError('out_channels must be divisible by groups')
...
class BitPruningLinear(BitPruningModule):
    def __init__(self, in_features, out_features, bias=True, w_bit=-1, a_bit=-1, half_wave=True):
        super(BitPruningLinear, self).__init__(w_bit=w_bit, a_bit=a_bit, half_wave=half_wave)
        self.in_features = in_features
        self.out_features = out_features
        self.weight = nn.Parameter(torch.zeros(out_features, in_features))
        if bias:
