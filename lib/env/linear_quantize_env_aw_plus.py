...
from lib.utils.quantize_utils import QConv2d, QLinear, calibrate, LSQConv2d, LSQLinear


class LinearQuantizeEnvAWPlus:
    def __init__(self, model, pretrained_model, data, data_root, compress_ratio, flash_memory_compress_ratio,  args, n_data_worker=16,
                 batch_size=256, float_bit=8, is_model_pruned=False, reward_strategy : int = 0):
        # default setting
        self.quantizable_layer_types = [QConv2d, QLinear, LSQConv2d, LSQLinear]
        ...
        self.flash_memory_compress_ratio = flash_memory_compress_ratio
        ...
        # sanity check
        assert self.compress_ratio > self._min_cost() / self._org_cost() and self.flash_memory_compress_ratio > self._min_weight() / self._org_weight(), \
    ...
    def step(self, action):
        ...
        if self._is_final_layer() and (not self.action_radio_button):
            ...
            a_size = self._cur_activation()
            a_size_ratio = self._cur_activation() / self._org_activation()
            ...
            reward = 0
            if self.reward_strategy == 0:
                # reward = self.reward(acc)
                reward = self.reward(acc, w_size_ratio, a_size_ratio)
            elif self.reward_strategy == 1:
                reward = self.releQ_reward(acc, cost, w_size)
            else:
                reward = self.autoQ_reward(acc, cost, w_size)

            info_set = {'cost_ratio': cost_ratio, 'w_size_ratio': w_size_ratio, 'accuracy': acc, 'cost': cost, 'w_size': w_size}

            if reward > self.best_reward:
                self.best_reward = reward
                prGreen('New best policy: {}, reward: {:.3f}, acc: {:.3f}, cost_ratio: {:.3f}, weight_ratio: {:.3f}'.format(
                    self.strategy, self.best_reward, acc, cost_ratio, w_size_ratio))
        ...
        cost = self._cur_cost()
        w_size = self._cur_weight()
        info_set = {'cost': cost, 'w_size': w_size}
        reward = 0
        done = False
    ...
    # for quantization
    def reward(self, acc, w_cost_ratio=None, a_cost_ratio=None, alpha = 0.75):
        if w_cost_ratio is not None and a_cost_ratio is not None:
            return (acc - self.org_acc + alpha * 1. / w_cost_ratio \
                + alpha * 1. / a_cost_ratio) * 0.1
        return (acc - self.org_acc) * 0.1
    
    # 
    def releQ_reward(self, acc, latency, memory):
        state_quantization = (latency + memory * 120) / (self._org_cost() +  self._org_weight() * 120)
        reward = 1 - state_quantization ** 0.2
        state_acc = acc * 1.0 / self.org_acc
        if state_acc < 0.4:
            reward = (acc - self.org_acc) * 0.1
        else:
            reward = reward * (state_acc ** (0.4 / state_acc))
        return reward
    
    def autoQ_reward(self, acc, latency, memory):
        return math.log((acc ** 2)/ ((latency ** 0.5) * (memory ** 0.5)))
    ...
    def _final_action_wall(self):
        ...
        target_memory = self.flash_memory_compress_ratio * self._org_weight()
        ...
        while (min_cost < self._cur_cost() or min_weight < self._cur_weight()) and (target < self._cur_cost() or target_memory < self._cur_weight()):
            # print('current: ', self.strategy, min_cost, self._cur_cost())
            for i, n_bit in enumerate(reversed(self.strategy)):
                if n_bit[1] > self.min_bit and target < self._cur_cost():
                    self.strategy[-(i+1)][1] -= 1
                self._keep_first_last_layer()
                if target >= self._cur_cost() and target_memory >= self._cur_weight():
                    break
                ...
                if target >= self._cur_cost() and target_memory >= self._cur_weight():
                    break
        print('after action_wall: ', self.strategy, 
              ' min time cost is ', min_cost, 'target time cost is', target, 
              ' cur time cost is', self._cur_cost(), ' min memory cost is', min_weight, 
              'target memory cost is', target_memory, ' cur memory cost is', self._cur_weight())
    ...
    def _org_weight(self):
        org_weight = 0.
        org_weight += sum(self.wsize_list[:-1]) * self.float_bit
        return org_weight
    ...
    def _org_activation(self):
        org_activation = np.array(self.asize_list[:-1]).sum(axis=1).max() * self.float_bit
    ...
    def _get_activation_size(self):
        ...
        for ind in self.quantizable_idx:
            ...
            if type(m) == nn.Conv2d or type(m) == QConv2d or type(m) == LSQConv2d:
                this_asize.append([m.in_w*m.in_h*m.in_channels, m.out_w*m.out_h*m.out_channels])
            elif type(m) == nn.Linear or type(m) == QLinear or type(m) == LSQLinear:
                this_asize.append([m.in_h, m.weight.shape[0]])
            self.asize_list.apend(np.hstack(this_asize))
        self.asize_dict = {i: s for i, s in zip(self.quantizable_idx, self.asize_list)}
        print("Activation peack memory: ", self.asize_list)
                  
    
    def _cur_weight(self):
        cur_weight = 0.
        # quantized
        for i, n_bit in enumerate(self.strategy[1:-1]):
            cur_weight += n_bit[0] * self.wsize_list[i + 1]
        # cur_weight += self.float_bit * (self.wsize_list[0] + self.wsize_list[-1])
        cur_weight += self.float_bit * (self.wsize_list[0])
        return cur_weight
    
    def _min_weight(self):
        # cur_weight = sum(self.wsize_list[1:-1]) * 1.0 * self.min_bit \
        #                 + self.float_bit * (self.wsize_list[0] + self.wsize_list[-1])
        cur_weight = sum(self.wsize_list[1:-1]) * 1.0 * self.min_bit \
                        + self.float_bit * (self.wsize_list[0])
        return cur_weight
    
    def _cur_activation(self):
        cur_activation = self.asize_list[0][0] * self.float_bit \
                            + self.asize_list[0][1] * self.strategy[1][1]
        
        for i in range(1, len(self.strategy) - 1):
            cur_activation = max(cur_activation, 
                                    self.asize_list[i][0] * self.strategy[i][1] \
                                        + self.asize_list[i][1] * self.strategy[i + 1][1])
        return cur_activation
    
    def _min_activation(self):
        cur_activation = np.array(self.asize_list[1:-1]).sum(axis=1).max() * self.min_bit
        cur_activation = max(cur_activation, sum(self.asize_list[0]) *  self.float_bit)
        return cur_activation
    ...
    def _cur_cost(self):
        ...
        # quantized
        for i, n_bit in enumerate(self.strategy[1:-1]):
            cur_cost += self.cost_lookuptable[i + 1, n_bit[0]-1, n_bit[1]-1]
        cur_cost += (self.cost_lookuptable[0, -1, -1] + self.cost_lookuptable[-1, -1, -1])
        return cur_cost
